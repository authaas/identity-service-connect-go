...
import (
	...
	"github.com/authaas/identity-service-connect-go/internal/service"
)
...
func Run() int {
	...
	svcCfg := svc.Configuration{Name: "identity"}
	...
	// Nothing is served without the data service, which is discovered, so the
	...
	// for Delete is held and watched, and every call goes to it.
	discovery := discover.New(serveCtx, log, conn, pbrpcotel.NewTransport(base))
	httpClient := &http.Client{Transport: discovery.Held()}

	upstream, err := discovery.Upstream(discover.URL(dataconnect.ServiceDeleteProcedure))
	if err != nil {
	...
	methodList, err := service_lib.Register(
		...
		func(rpc *connect.Server) {
			identityconnect.RegisterServiceHandler(rpc, server)
		},
	)
	if err != nil {
